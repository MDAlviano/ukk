            ...
            @foreach($carts as $item)
                <tr class="bg-white rounded-lg drop-shadow-lg">
                    <td class="py-4 px-8 align-middle">
                        <img src="{{ asset('/storage/' . $item->products->image_url) }}" alt="product image" class="w-20 rounded-md object-cover">
                    </td>
                    ...
                    <td class="py-4 align-middle">Rp{{ number_format($item->products->price, 0, ',', '.') }}</td>
                    <td class="py-4 align-middle">{{ $item->quantity }}</td>
                    <td class="py-4 px-8 align-middle">
                        <div class="flex gap-3">
                            <form action="{{ route('cart.remove', ['productId' => $item->products->id]) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
