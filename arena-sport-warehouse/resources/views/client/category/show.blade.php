...
<head>
    ...
    <title>{{ $category->name }} | Arena Sport Warehouse</title>
</head>
<body>
{{-- navbar --}}
<nav class="w-full flex flex-col">
    ...
    <div class="px-20 py-6 outline-1 outline-gray-300 flex flex-row justify-between items-center">
        ...
        <div class="flex flex-row w-full gap-4 px-3 py-2 rounded-lg outline-1 outline-dark-gray">
            ...
            <input type="text" name="q" placeholder="Search product here..." class="focus:outline-0 w-full">
        </div>
...
<main class="py-10 px-16 flex flex-col gap-16">
    {{-- filter & sort --}}
    <form method="GET" action="{{ route('categories.show', $category->slug) }}" class="flex flex-row w-full justify-between">
        @csrf
