@foreach($products as $product)
    <tr class="bg-white rounded-lg drop-shadow-lg">
        <td class="py-4 px-8 align-middle">
            <img src="{{ asset('/storage/' . $product->image_url) }}" alt="{{ $product->name }}" class="w-20 rounded-md object-cover">
        </td>
        ...
        <td class="py-4 px-8 align-middle">
            <div class="flex gap-3">
                <a href="{{ route('admin.products.edit', ['slug' => $product->slug]) }}" class="bg-[#E6E6E6] px-3 py-1 rounded-lg hover:bg-gray-300 transition duration-200">Edit</a>
                <form action="{{ route('products.delete', ['id' => $product->id]) }}"
                      method="POST" onsubmit="return confirm('Yakin hapus produk {{ $product->name }}?')">
                    @csrf
                    @method('DELETE')
